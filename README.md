...
- ✅ **任务系统**：优先级队列、状态机、失败重试、定时调度（cron）、智能跳转
- ✅ **AI精炼**：摘要+关键词+质量评分（0-100），OpenAI兼容接口，重新精炼，内容预处理
- ✅ **分类管理**：独立精炼配置、质量评分标准、预设模板
- ✅ **通知管理**：飞书（消息卡片/文本）/Telegram/Webhook，即时/聚合两种模式
- ✅ **兴趣图谱**：用户反馈（点赞/收藏/踩/批注）、兴趣点管理、AI发现、词云可视化
- ✅ **前端界面**：信源、分类、任务、结果、通知渠道、兴趣图谱全套管理页面
